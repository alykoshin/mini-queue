...
var JobQueue = function(config) {
  ...
  const maxJournalLength = 100;
  self.journal = {};//This will journal a job upon completion.
  ...
  // jobs at some states (dequed, rejected etc) can not be found by this method
  ...
  self.createJob = function(data) {
    ...
    if (!self.journal.hasOwnProperty(job.data.group)) {self.journal[job.data.group] = {};}
    if (!self.journal[job.data.group].hasOwnProperty(job.data.name)) {self.journal[job.data.group][job.data.name] = [];}
    job.log('createJob()');
  ...
  self._onJobComplete = function(job, err, result) {
    ...
    self.journal[job.data.group][job.data.name].unshift(new Date());
    self.journal[job.data.group][job.data.name].splice(maxJournalLength - 1);//Clip journal length
