...
const JobQueue = function(config) {
 ...
 self._getJobQueue = function(jobId) {
  ...
  self._queueJob = function(job) {
    ...
    job._toState('queue', function(err, result) {
      return self._onJobCancel(job, err, result);
    });
  ...
  self._startJob = function(job) {
    ...
    job._toState('process', function(err, result) {
  ...
  self._onJobComplete = function(job, err, result) {
    job.log('Queue._onJobComplete(): err: %j, result: %j', err, result);
    ...
    job._toState('complete', err, result);
